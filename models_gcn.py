...
class GCN(torch.nn.Module):
    ...
    def forward(self, data, if_embed=False, if_attention = False, dist=False,p = 1):
 ...
 #            if if_attention:
        ...
        #    return attention_outs
            if if_attention:
              squared_tensor = x** p
              sq = torch.matmul(squared_tensor.t(),squared_tensor)
              sq = sq/torch.norm(sq,1)
              attention_outs.append(sq)
    ...
    ###########################
    def attention(self, data, power=1):
        p = power
        return self.forward(data, if_attention=True,p = p)
    def distribution(self, data):
        return self.forward(data, dist = True)
    ############################
