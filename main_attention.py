...
from graph_agent_attention_matching import GraphAgent
import argparse
import random
import numpy as np
from utils import *
...
parser.add_argument('--lr_dist_matching', type=float, default=0.1)
...
##################################################
parser.add_argument('--lr_dist', type=float, default=0.1)
parser.add_argument('--attention_power', type=float, default=2)
...
agent = GraphAgent(data=packed_data, args=args, device=device, nnodes_syn=get_mean_nodes(args),attention_power=args.attention_power)
